...
namespace cam_matrix::ui
{
    ...
    CameraControlWidget::CameraControlWidget(QWidget *parent)
        : QWidget(parent), formatGroup_(nullptr), formatCombo_(nullptr), captureGroup_(nullptr), captureButton_(nullptr), hqCaptureButton_(nullptr), highQualityCheckbox_(nullptr), currentCameraIndex_(-1), currentFormat_("tiff")
    {
        setupUi();
        createConnections();
    }
    ...
    void CameraControlWidget::setupUi()
    {
        ...
        formatCombo_->addItems({"1920x1080 (MJPEG)",
                                "1280x720 (MJPEG)",
                                "640x480 (MJPEG)",
                                "1920x1080 (YUV)",
                                "1280x720 (YUV)",
        ...
        // Format selection for high-quality capture
        auto *formatHQLayout = new QHBoxLayout();
        auto *formatLabel = new QLabel(tr("HQ Format:"), this);

        QComboBox *hqFormatCombo = new QComboBox(this);
        hqFormatCombo->setStyleSheet(comboStyle);
        hqFormatCombo->addItems({"TIFF",
                                 "PNG (Best Quality)",
                                 "BMP"});

        formatHQLayout->addWidget(formatLabel);
        formatHQLayout->addWidget(hqFormatCombo);
        captureLayout->addLayout(formatHQLayout);

        // Connect the format combo box
        connect(hqFormatCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
                this, [this, hqFormatCombo](int index)
                {
                QString format = hqFormatCombo->itemText(index);
        ...
        // High-quality capture button
        hqCaptureButton_ = new QPushButton(tr("Capture High-Quality Photo"), this);
        hqCaptureButton_->setStyleSheet(buttonStyle);
        hqCaptureButton_->setEnabled(false); // Initially disabled until camera connected
        captureLayout->addWidget(hqCaptureButton_);
    ...
    void CameraControlWidget::createConnections()
    {
        ...
        connect(hqCaptureButton_, &QPushButton::clicked,
                this, &CameraControlWidget::onHighQualityCaptureClicked);
    }
    ...
    void CameraControlWidget::setCameraIndex(int index)
    {
        ...
        // Enable capture buttons when a camera is selected
        captureButton_->setEnabled(index >= 0);
        hqCaptureButton_->setEnabled(index >= 0);
    ...
    void CameraControlWidget::onHighQualityCaptureClicked()
    {
        if (currentCameraIndex_ < 0)
            return;

        emit statusChanged(tr("Capturing high-quality photo (%1) from camera %2...")
                               .arg(currentFormat_)
                               .arg(currentCameraIndex_));
        emit highQualityPhotoCaptureRequested(currentFormat_);
    }
    ...
    void CameraControlWidget::setEnabled(bool enabled)
    {
        ...
        // Make sure the capture buttons are enabled appropriately
        captureButton_->setEnabled(enabled && currentCameraIndex_ >= 0);
        hqCaptureButton_->setEnabled(enabled && currentCameraIndex_ >= 0);
