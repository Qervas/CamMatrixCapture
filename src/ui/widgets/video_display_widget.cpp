...
namespace cam_matrix::ui
{
    ...
    VideoDisplayWidget::VideoDisplayWidget(QWidget *parent)
        : QWidget(parent), frameUpdated_(false)
    {
    ...
    void VideoDisplayWidget::updateFrame(const QImage &frame)
    {
        try
        {
            // First validate the frame to prevent crashes
            if (frame.isNull())
            {
                qWarning() << "Received null frame in VideoDisplayWidget::updateFrame";
                // Don't attempt to process a null frame, but don't clear either
                return;
            }

            // Check for invalid frames
            if (frame.width() <= 0 || frame.height() <= 0 || !frame.constBits())
            {
                qWarning() << "Received invalid frame dimensions:" << frame.width() << "x" << frame.height();
                return;
            }

            // Copy the frame safely with depth verification
            QImage frameCopy;
            try
            {
            ...
            {
                qWarning() << "Exception during frame.copy() - frame may be corrupted";
                return;
            }
            ...
            if (frameCopy.isNull())
            {
                qWarning() << "Failed to copy frame - result is null";
                return;
            }
